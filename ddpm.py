...
model_summary = summary(
    model,
    input_data=[torch.randn(1, 1, 28, 28).to(device), torch.tensor([0]).to(device)],
    depth=5,
    verbose=0,
    col_names=["input_size", "output_size", "num_params"],
)

# 后处理：对于参数为 -- 的行，隐藏 input 和 output
def filter_no_param_layers(summary_str):
    """对于参数为 -- 的行，将 input_size 和 output_size 列替换为空"""
    ...
    for line in lines:
        ...
        if line.rstrip().endswith('--'):
            # 使用正则表达式匹配并替换 [x, x, x, x] 格式的形状信息
            # 保留层名称部分，清空形状信息
            line = re.sub(r'\[[\d, ]+\]', lambda m: ' ' * len(m.group()), line)
        result_lines.append(line)
    
    return '\n'.join(result_lines)

filtered_summary = filter_no_param_layers(str(model_summary))

with open("model_summary.txt", "w", encoding="utf-8") as f:
    f.write(filtered_summary)
print("模型结构已保存到 model_summary.txt")
